...
from notebooklm_tools.core.conversation import NotebookConversation

def main():
    if len(sys.argv) < 3:
        print("Usage: nlm_query.py <notebook_id> <query_text>", file=sys.stderr)
        sys.exit(1)
    ...
    conv = NotebookConversation(client)
    result = conv.query(notebook_id, query_text, timeout=180.0)
